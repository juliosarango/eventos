...
from app.apps.utils.enums import TipoEmail
from app.apps.taskapp.task import send_email_for_user
...
def activar_usuario(request, uidb64, token):
    ...
    if user and default_token_generator.check_token(user, token):
        if not user.is_active:
            user.is_active = True
            user.save()
            message = "Gracias por confirmar su email, su cuenta ha sido habilitada. " "Ya puede ingresar al sistema."
        else:
